/* ═══════════════════════════════════════════════════════════
   Algebra 2 — Firebase Auth + Firestore Sync
   Google Sign-In, cross-device progress sync
   ═══════════════════════════════════════════════════════════ */

const Auth = (() => {
  const USERS_COL   = 'alg2_users';
  const STORAGE_KEY = 'alg2_progress_v1';
  const SYNC_KEY    = 'alg2_firebase_synced'; // sessionStorage flag to prevent reload loops
  ...
  function init() {
    ...
    // We use window.onProgressSaved (set here, called from app.js) so that
    // every localStorage write also flows through to Firestore.
  ...
  // Fires whenever another device writes progress. Updates localStorage
  // and dispatches 'progress:synced' so the SPA can re-render.
  function watchProgress(user) {
    ...
    unsubscribeWatch = db.collection(USERS_COL).doc(user.uid)
      .onSnapshot((doc) => {
        ...
        // hasPendingWrites = true means this snapshot is from our own write
        // being echoed back — skip to avoid unnecessary re-renders.
  ...
  async function syncFromCloud(user) {
    if (sessionStorage.getItem(SYNC_KEY)) {
      // Already did the initial load this session — just watch for changes
      watchProgress(user);
      return;
    }
    ...
    try {
      ...
      if (doc.exists && doc.data().progress &&
        ...
        // Cloud data exists → load into localStorage and reload once
        localStorage.setItem(STORAGE_KEY, JSON.stringify(doc.data().progress));
        sessionStorage.setItem(SYNC_KEY, '1');
        window.location.reload();
      } else {
        // First login → migrate any existing local progress to Firestore
        const localStr      = localStorage.getItem(STORAGE_KEY) || '{}';
  ...
  // ─── Simple toast (reuses app toast if available) ──────
